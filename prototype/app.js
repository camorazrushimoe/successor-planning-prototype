...
function toP(list){
  ...
  list.forEach(p=>{
    ...
    o[p.id]={
      n:p.name,t:p.title||p.level,b:boxOf(p.performance,p.potential),
      w:p.notes||(fb.score?("client "+fb.score):""),
      level:p.level,track:p.track,raw:p
    };
  });
  return o;
}
...
{id:"n",name:"DP — Nordics",inc:"demo-inc",pl:[],rec:["demo-star","demo-diamond","demo-expert","demo-split"],em:null,hz:{},dis:[],goals:{"demo-star":[{t:"QBR без партнёра",due:"2026-10-15",s:"open",by:"head"}]}},
...
function mine(){const id=S.cand;if(!P[id])return"<p class='how'>нет профиля</p>";const rs=S.roles.filter(r=>r.pl.some(x=>x.id===id)||r.rec.includes(id));
 return `<p class="how">Кабинет кандидата. Только свои роли.</p><h2>${P[id].n}</h2><div class="grid">${rs.map(r=>{const g=gs(r,id);return `<article class="card"><h3>${r.name}</h3><span class="chip">${r.pl.some(x=>x.id===id)?"planned":"recommended"}</span> <span class="chip">${r.hz[id]||"—"}</span>${g.map((x,i)=>`<div class="slate-item">${x.t} · ${x.due||"без даты"} · ${x.s} <button onclick="gs(R('${r.id}'),'${id}')[${i}].s='done';draw()">готово</button> <button onclick="S.modal={t:'goal',rid:'${r.id}',pid:'${id}',i:${i}};draw()">дата</button></div>`).join("")}<button class="acc" onclick="S.modal={t:'goal',rid:'${r.id}',pid:'${id}'};draw()">+ цель</button></article>`}).join("")||"<p class='why'>нет ролей</p>"}</div>`}
function openP(id,rid){S.person={id,rid};draw()}
function card(){if(!S.person||S.role==="candidate"||!P[S.person.id])return"";const p=P[S.person.id],r=R(S.person.rid)||S.roles[0],g=gs(r,S.person.id);
 return `<div class="modal" onclick="if(event.target===this){S.person=null;draw()}"><div class="box"><button onclick="S.person=null;draw()">закрыть</button><h3>${p.n}</h3><div class="why">${r.name} · ${p.t} · ${L[p.b]}</div>${g.map((x,i)=>`<div class="slate-item">${x.t} · ${x.due||"—"} · ${x.s} ${C[S.role].go?`<button onclick="gs(R('${r.id}'),'${S.person.id}').splice(${i},1);draw()">удалить</button>`:""}</div>`).join("")}${C[S.role].go?`<button class="acc" onclick="S.modal={t:'goal',rid:'${r.id}',pid:'${S.person.id}'};draw()">+ цель</button>`:""}${C[S.role].hz?["ready_now","6m","12m","18m"].map(h=>`<button onclick="hz('${r.id}','${S.person.id}','${h}')">${h}</button>`).join(""):""}${C[S.role].cm?`<textarea id="cmt" placeholder="коммент"></textarea>`:""}</div></div>`}
function poolOpts(rid){
  ...
  return ids().filter(id=>!used.has(id)).filter(id=>{
    const p=P[id];return !q||p.n.toLowerCase().includes(q)||(p.t||"").toLowerCase().includes(q)||id.toLowerCase().includes(q);
  });
}
function mods(){if(!S.modal)return"";
 ...
 if(S.modal.t==="pool"){const opts=poolOpts(S.modal.rid);return `<div class="modal"><div class="box"><h3>В recommended · пул ${ids().length}</h3><input id="pq" placeholder="поиск по имени / title / id" value="${S.q||""}" oninput="S.q=this.value;draw()"/><select id="pool" size="8">${opts.map(id=>`<option value="${id}">${P[id].n} — ${P[id].t}</option>`).join("")}</select><p class="why">${opts.length?opts.length+" чел.":"никого"}</p><button onclick="S.modal=null;draw()">нет</button><button class="acc" onclick="addFromPool()">ок</button></div></div>`}
...
function svR(){const name=rn.value||"Роль",inc=ri.value;if(S.modal.id){const r=R(S.modal.id);r.name=name;r.inc=inc;r.pl=r.pl.filter(x=>x.id!==inc);r.rec=r.rec.filter(i=>i!==inc)}else S.roles.unshift({id:"r"+Date.now(),name,inc,pl:[],rec:[],em:null,hz:{},dis:[],goals:{}});S.modal=null;draw()}
...
async function boot(){
  try{
    ...
    if(res.ok){
      ...
      if(list.length){P=toP(list);if(!P[S.cand])S.cand=ids()[0]}
    }
  }catch(e){}
  draw();
}
